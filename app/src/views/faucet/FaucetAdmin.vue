<template>
  <wallet-connect/>
  <main-section v-if="sessionId != null && role === 'admin'">
    <FaucetCreate></FaucetCreate>
    <tiles>
      <card-component title="Topup Token Faucet" :icon="mdiAccountCircle" class="tile is-child">
        <div class="flex justify-center md-8">
          <figcaption class="text-sm text-gray-600">
            <cite>Topup token faucet with tokens. # of token transferred to each address = total amount / no of addresses in the faucet</cite>
          </figcaption>
        </div>
...
export default {
  ...
  methods: {
    async buildTransaction() {
      ...
      try {
        res = await fetch(`${api_base_url}/faucet/${this.faucet}/topup`, {
          method: 'POST',
          headers: {
      ...
      let txnResponse;
      try {
        txnResponse = await fetch(`${api_base_url}/faucet/${this.faucet}/topup/transfer`, {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
